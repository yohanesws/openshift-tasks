node('maven') {
  ...
  def sourceUrl = "https://github.com/yohanesws/openshift-tasks"
  ...
  stage 'deployInDev'
    ...
    sh "oc new-build --name=${applicationName} --image-stream=jboss-eap70-openshift:1.6 --binary=true --labels=application=${applicationName} -n ${devProject} || true"
    // build image
